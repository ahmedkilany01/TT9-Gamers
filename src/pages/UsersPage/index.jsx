...
import { AUTH_API_ENDPOINT } from "../../router/paths";
...
const UsersPage = () => {
  ...
  const { getUsers, users, isLoading, deleteUser } = UseApi(AUTH_API + AUTH_API_ENDPOINT.USERS + sizeQuery + searchQuery + pageQuery, config);
  ...
  return (
    <div className={Style.users}>
      ...
      <Table columns={TABLE_COLUMNS(onDeleteHandler)} data={users?.users} isLoading={isLoading} />
      <div className={`${Style.pageNumContainer} ${Style.pagination_buttons}`}>
        <button onClick={prevSubmit} disabled={users.page === 1}>
          prev
        </button>
        {users.pages > 1 && (
          <>
            {users.page > 3 && <span className={Style.pagination_points}>...</span>}
            {pageNumbers.map((pageNumber) => {
              if (pageNumber === 1 || pageNumber === users.pages || (pageNumber >= users.page - 2 && pageNumber <= users.page + 2)) {
                return (
                  <button key={pageNumber} onClick={() => setNum(pageNumber)} className={users.page === pageNumber ? Style.active_page : ""}>
                    {pageNumber}
                  </button>
                );
              }
            ...
            {users.page < users.pages - 2 && <span className={Style.pagination_points}>...</span>}
          </>
        )}
