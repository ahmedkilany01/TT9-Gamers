...
const QuoteLogin = () => {
  ...
  return (
    <Container>
      <section className={Style.left__div}>
        <div className={Style.left__div__text}>
          <Typography variant={"h2"}>Join the game!</Typography>
          <Typography variant={"h5"}>Go inside the best gamers social network!</Typography>
        </div>
        ...
        <Or />
        <form onSubmit={handleSubmit(onHandleSubmit)} className={Style.Wrapped__form__group}>
          {inputs.map((input, index) =>
            input.type == "email" ? (
              <div key={input.id} className={Style.form__group}>
                ...
                <input {...register("email")} id={input.id} name={input.name} type={input.type} className={Style.form__input} placeholder={input.placeholder} />
                {errors.email && <span className={Style.errors}>{errors.email.message}</span>}
              </div>
            ) : (
              <Fragment key={input.id}>
                <div className={Style.form__input__password}>
                  <label htmlFor={input.id} className={Style.form__label}>
                    <Typography variant={"h6"}>{input.label}</Typography>
                  </label>
                  <input
                  ...
                  {showPassword ? (
                    ...
                    <Image handleShowPassword={handleShowPassword} ImageSrc="/assets/Vector.svg" />
                  )}
                </div>
