...
const useAuth = () => {
  ...
  const login = async (body) => {
    ...
    try {
      const { data } = await axios.post(AUTH_API + AUTH_API_ENDPOINT.LOGIN, body);
      console.log(data);
  ...
  const signup = async (body) => {
    dispatch({ type: AUTH_ACTIONS.SET_LOADING });
    try {
      const { data } = await axios.post(AUTH_API + AUTH_API_ENDPOINT.SIGNUP, body);
      dispatch({ type: AUTH_ACTIONS.AUTHORIZE, payload: { user: data } });
    } catch (error) {
      dispatch({ type: AUTH_ACTIONS.SET_ERROR, payload: error.message });
    }
  ...
  const getProfileData = async () => {
    dispatch({ type: AUTH_ACTIONS.SET_LOADING });
    try {
      const { data } = await axios.get(AUTH_API + AUTH_API_ENDPOINT.PROFILE, config);
      dispatch({ type: AUTH_ACTIONS.AUTHORIZE, payload: { user: data } });
    } catch (error) {
      dispatch({ type: AUTH_ACTIONS.SET_ERROR, payload: error.message });
    }
